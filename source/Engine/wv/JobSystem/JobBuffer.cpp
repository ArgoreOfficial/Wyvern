...
{
	int32_t head = m_head.load( std::memory_order::acquire );

	if ( head >= static_cast<int32_t>( m_jobs.size() ) )
		return false; // queue is full
	
	m_jobs[ head ] = _pJob;
	m_head.store( head + 1, std::memory_order::release );
...
wv::Job* wv::JobBuffer::pop()
{
	...
	int32_t tail = m_tail.load( std::memory_order::acquire );

	if ( tail > head )
	{
		// queue is empty
		m_head.store( tail, std::memory_order::release );
		return nullptr;
	}
	...
	if ( !m_tail.compare_exchange_weak( tail, tail + 1, std::memory_order::acq_rel ) )
		job = nullptr; // failed race against steal
		
	m_head.store( tail + 1, std::memory_order::release );
