...
{
    return pow( _col, vec3( 1.0 / uGamma ) );
}

vec3 tonemap( vec3 _col )
{
    return vec3(1.0) - exp(-_col * uExposure);
}
...
vec3 combineRender() 
{
	...
	albedo.xyz  = pow    ( albedo.xyz, vec3( uGamma ) ); // gamma correction
...
void main()
{
	...
	else if( uRenderMode == 2 ) FragColor.rgb = texture( light_Normal,            TexCoord ).rgb * 0.5f + vec3( 0.5f, 0.5f, 0.5f );
